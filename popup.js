...
var PassHashCommon =
{
    ...
    generateHashWord: async function(
                ...
                restrictDigits)
    {
        async function new_b64_hmac_sha1(masterKey, siteTag) {
            const enc = new TextEncoder('utf-8');
            const key = await window.crypto.subtle.importKey('raw', enc.encode(masterKey), { name: 'HMAC', hash: {name: 'SHA-1'}}, /*export=*/false, ['sign', 'verify']);
            const signature = await window.crypto.subtle.sign('HMAC', key, enc.encode(siteTag));
            const digest = btoa(String.fromCharCode(...new Uint8Array(signature))).replace(/=*$/g, '');
            return digest
        }

        // Start with the SHA1-encrypted master key/site tag.
        var s = await new_b64_hmac_sha1(masterKey, siteTag);
